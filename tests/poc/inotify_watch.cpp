...
enum { FD_POLL_SIGNAL = 0, FD_POLL_INOTIFY, FD_POLL_MAX };
...
static void __event_process(struct inotify_event *event) {
  int i;
...
static int __initialize_inotify(int argc, const char **argv) {
  int i;
  int inotify_fd;
...
static void __shutdown_signals(int signal_fd) { close(signal_fd); }
...
int main(int argc, const char **argv) {
  ...
  for (;;) {
    ...
    if (fds[FD_POLL_INOTIFY].revents & POLLIN) {
      ...
      if ((length = read(fds[FD_POLL_INOTIFY].fd, buffer,
